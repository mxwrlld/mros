...
class SVM:
    def __init__(self, training_sample, threshold):
        ...
        P = np.loadtxt(
            "B:\Study\9_semestr\МРО (Мясников. Баврина)\mros\data\Ps\P.txt")
        ...
        np.savetxt(
            "B:\Study\9_semestr\МРО (Мясников. Баврина)\mros\data\Ps\L_M.txt", self.lyambdas)
    ...
    def calc_w_n(self):
        if self.support_vectors.shape[1] != 0:
            return np.mean(
                self.support_vectors[3] - self.w.T @ self.support_vectors[0:2])
